...
function completePasswordReset(req, done) {
  ...
  // console.log('getProperty(req, emailProperty) =', getProperty(req, emailProperty))

  var reset_code = getProperty(req, 'code'),
      password = getProperty(req, 'password'),
      email = getProperty(req, emailProperty),
      now = Date.now();
  ...
  if (!email) {
    return done(new Errors.BadRequest('Missing User Email'));
  }
  ...
  async.waterfall([
    ...
    function sendPasswordResetNotificationEmail (callback) {
      mailer('passwordresetsuccess', 'en_US', {
        email: email,
        code: true,
        request: req
      }, email, password_reset_success_subject, callback);
    },
...
function createCafeAuth(req, done) {
  ...
  async.waterfall ([
    ...
    function processUser(cafeOwner, callback) {
      if (cafeOwner) {
        ...
        auth = {
          ...
          CafePlanId: req.body.CafePlanId
        }
    ...
    function sendAuthCodeToTheUser(user, callback) {
      if (!callback && typeof user === 'function') {
        callback = user;
      }
...
module.exports = {
  ...
  init: function(config) {
    ...
    password_reset_success_subject = config.password_reset_success_subject;
