...
var PASSWORD_RESET_SUBJECT = 'Reset Password';
var ACTIVATION_SUBJECT = 'Activate Your Account';
...
function getProperty(req, propertyName) {
  return req.activator && req.activator[propertyName] ||
    req.params && req.params[propertyName] ||
    req.body && req.body[propertyName] ||
    req.query && req.query[propertyName];
}
...
module.exports = {
  ...
  init: function(config) {
    ...
    PASSWORD_RESET_SUBJECT = config.password_reset_subject || PASSWORD_RESET_SUBJECT;
    ACTIVATION_SUBJECT = config.activation_subject || ACTIVATION_SUBJECT;

    // setup mailer
    if (typeof config.smtp === 'function') {
      mailer = config.smtp;
    } else {
      _.extend(config.smtp, _.pick(config, 'protocol', 'domain', 'pathActivate', 'pathResetPassword'));
      mailer = smtp(config.smtp, config.from, templates);
    }

    // user properties
    emailProperty = config.emailProperty || DEFAULTS.emailProperty;
    idProperty = config.idProperty || DEFAULTS.idProperty;
