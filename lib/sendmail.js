...
module.exports = function (options) {
  ...
  if (options.transport) {
    debug('using transport passed to options');
    transport = options.transport;
  } else {
    // create reusable transport method (opens pool of SMTP connections)
    transport = nodemailer.createTransport(smtpPool(options.smtp));
  }
  ...
  from = from || options.auth && options.auth.user;
  if (!from) {
  ...
  var sendMail = function (to, subject, html, callback) {
    ...
    transport.sendMail(opts, function (err, info) {
      if (err) {
        console.log('err =', err)
        return callback(new Errors.Common('Couldn\'t send email', err.responseCode || 500));
      }
